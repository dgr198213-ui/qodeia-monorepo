-- =============================================================
-- 0001 · Núcleo operativo del agente (consolidación Fase 3A)
-- Origen: packages/agent-core/supabase/master_schema.sql
-- Cambios respecto al original:
--   * Sustituido el patrón RLS `current_setting('request.role')` (no
--     funciona en Supabase: el service_role bypasea RLS y ese setting
--     no se popula) por `false` explícito — la escritura queda
--     reservada al backend con service key.
-- =============================================================
...
CREATE POLICY "Servicio puede gestionar memoria vectorial"
    ...
    USING (false /* solo backend service-role, que bypasea RLS */);
