...
interface MCPConfig {
  ...
  howard_os_supabase_url: string;
  howard_os_supabase_key: string;
  enabled: boolean;
  last_updated: string;
}
    ...
    try {
      ...
      await fetch('/api/mcp/update-env', {
        ...
        body: JSON.stringify({
          ...
          HOWARD_OS_SUPABASE_URL: config.howard_os_supabase_url,
          HOWARD_OS_SUPABASE_KEY: config.howard_os_supabase_key,
        }),
      });
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 divide-y divide-gray-200">
          ...
          {/* 3. Howard OS Supabase Connection */}
          <div className="p-8">
            <h2 className="text-xl font-bold text-gray-900 mb-4">3. Conexión Supabase Howard OS</h2>
            <p className="text-gray-600 mb-6">
              Configura las credenciales específicas para la base de datos de Howard OS.
            </p>
            <div className="space-y-6">
              <InputGroup
                label="Supabase URL (Howard OS)"
                value={config.howard_os_supabase_url}
                onChange={(v) => setConfig({ ...config, howard_os_supabase_url: v })}
                placeholder="https://xyz.supabase.co"
              />
              <InputGroup
                label="Supabase Service Key (Howard OS)"
                value={config.howard_os_supabase_key}
                onChange={(v) => setConfig({ ...config, howard_os_supabase_key: v })}
